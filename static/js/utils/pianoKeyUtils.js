...
(function() {
  ...
  define(['static/js/data/ChordData', 'static/js/data/PianoKeys', 'static/js/config/Config', 'static/js/utils/Logger'], function(ChordData, PianoKeys, Config, Logger) {
    var IntervalName, PianoKeyOctaveIndices, VALID_WHITE_KEY_NAMES, pianoKeyUtils, _findRecognizedClosedSpelling, _getChordNameFromSignature, _getIntervalSize, _getIntervalSizes, _getSignature, _identifyChord, _identifyInterval, _pianoKeyIdComparator, _rejectHigherDuplicatesOfLowerKeys;
    ...
    _getChordNameFromSignature = function(keys, signature) {
      ...
      if (chordData != null) {
        ...
        chordName = Config.DEBUG_MODE ? signature : Config.LABEL_FOR_UNRECOGNIZED_CHORDS;
      }
      return chordName;
    };
    ...
    pianoKeyUtils = {
      ...
      getKeyNameOfNextHighestWhiteKey: function(whiteKeyName) {
        var higherKeyIndex;
        if (!pianoKeyUtils.isValidWhiteKeyName(whiteKeyName)) {
          throw new Error("Invalid whiteKeyName " + whiteKeyName);
        }
        if (whiteKeyName === _.last(VALID_WHITE_KEY_NAMES)) {
          higherKeyIndex = 0;
        } else {
      ...
      getKeyNameOfNextLowestWhiteKey: function(whiteKeyName) {
        var lowerKeyIndex;
        if (!pianoKeyUtils.isValidWhiteKeyName(whiteKeyName)) {
          throw new Error("Invalid whiteKeyName " + whiteKeyName);
        }
        if (whiteKeyName === _.first(VALID_WHITE_KEY_NAMES)) {
      ...
      isValidWhiteKeyName: function(keyName) {
        return __indexOf.call(VALID_WHITE_KEY_NAMES, keyName) >= 0;
      },
